...
{
    return (static_cast<XMP_Uns32>(data[0]) << 0) | (static_cast<XMP_Uns32>(data[1]) << 8);
}

static inline XMP_Uns32 GetLE24(const XMP_Uns8* const data)
{
    return GetLE16(data) | (static_cast<XMP_Uns32>(data[2]) << 16);
}
...
{
    assert(val < (1 << 16));
    data[0] = static_cast<XMP_Uns8>(val >> 0);
    data[1] = static_cast<XMP_Uns8>(val >> 8);
}
...
static inline void PutLE24(XMP_Uns8* const buf, XMP_Uns32 val)
{
    ...
    buf[2] = static_cast<XMP_Uns8>(val >> 16);
}
...
#define WEBP_MKFOURCC(a, b, c, d)                                              \
    (static_cast<XMP_Uns32>(a) | static_cast<XMP_Uns32>(b) << 8 | static_cast<XMP_Uns32>(c) << 16 | static_cast<XMP_Uns32>(d) << 24)
